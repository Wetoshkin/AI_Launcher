...
- Логика HF-фильтрации вынесена из `HomeViewModel` в `RemoteModelFilterController`:
  - family/quant/size/capability фильтры применяются одной точкой;
  - поведение покрыто отдельными Desktop unit-тестами.
